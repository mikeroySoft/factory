...
| `factory plan list` / `factory plan inspect N` | Read-only schema 1 JSON over `initiative` issues: declared status/owner, parsed sections, `#N` implementation links (`inspect` also fetches each linked issue's title/state), per-issue `malformed` + `problems`, cited sources with `observed_at`. `list` reads at most 3 pages of 100; a failed page or malformed initiative yields `coverage.status: partial` and exit 1, never a mutation. |
