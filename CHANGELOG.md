...
- Routed human handoffs (#56): once automatic recovery for an escalation is terminal (manager `HUMAN`, manager command failure as an explicit unable-to-diagnose, `manager.rounds` exhausted, or no manager), `factory manage` publishes one public-safe request per escalation generation with the question, bounded evidence links, proposed next step and the `factory plan route` owner/candidates and rationale; mentions only on the initial handoff and on an actual owner change, never assigning anyone. Intent and comment id are journaled (`handoff`, `comment` rows); a crash or failed `gh` between them is reconciled from the timeline before any retry and a failed lookup posts nothing. Human-takeover checks now trust only recorded comment ids (escalation, manager and handoff comments), never text prefixes; an edited recorded comment is human activity. Escalations recorded before this version carry no receipt, so their pending manager rounds are left to the human.
